...
def sample(newcfg: DictConfig):
    ...
    logger.info(f"Model Configuration: {OmegaConf.to_yaml(cfg.model)}")
    # logger.info(f"Feature Predictor: {model.feature_predictor}")
    # logger.info(f"Feature Predictor Class: {type(model.feature_predictor)}")
    # logger.info(f"Feature Predictor Parameters: {model.feature_predictor.state_dict().keys()}")
    ...
    with torch.no_grad():
        with Progress(transient=True) as progress:
            ...
            for idx, keyid in enumerate(dataset.keyids):
                ...
                # print("###########################################\n\tbatch shape:",batch["shape"][0].size(),"\n################################")
                shape = batch['shape'][0][:, 0, :].numpy()
