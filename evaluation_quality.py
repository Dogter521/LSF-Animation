...
def heatmap(cfg: DictConfig):
    ...
    for i, directory in enumerate(pred_dir):
        ...
        seq_frames = seq_frames.reshape(-1, 5023, 3)
...
def render_heatmap(reference_path, template_all, motion_vec, output_path):
    ...
    ms.apply_filter('compute_color_from_scalar_using_transfer_function_per_vertex',
                    minqualityval=motion_vec.min(),
                    maxqualityval=motion_vec.max(), tfslist=2, brightness=1)
    tmp_mesh_path = f'{Path(output_path).parent}/tmp.obj'
    ms.save_current_mesh(tmp_mesh_path, save_vertex_color=True)

    ref_mesh = trimesh.load_mesh(tmp_mesh_path)
    py_mesh = pyrender.Mesh.from_trimesh(ref_mesh)

    scene = pyrender.Scene()
    node = pyrender.Node(mesh=py_mesh, translation=[0, 0, 0])
    scene.add_node(node)
    scene.add(cam, pose=camera_pose)
    scene.add(light, pose=camera_pose)
    color, _ = r.render(scene)

    # Normalize motion vector for colorbar
    norm = Normalize(vmin=motion_vec.min(), vmax=motion_vec.max())
    ...
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, shrink=0.6, aspect=20, pad=0.03)
